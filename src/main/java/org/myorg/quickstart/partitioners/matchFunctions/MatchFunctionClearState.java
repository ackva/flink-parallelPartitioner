...
public class MatchFunctionClearState extends KeyedBroadcastProcessFunction<Integer, Edge<Integer, Long>, Tuple2<HashMap<Integer, Integer>,Long>, Tuple2<Edge<Integer, NullValue>,Integer>> {
   ...
   //MapStateDescriptor<String, Tuple2<Long,Boolean>> updatedStateDescriptor = new MapStateDescriptor<>("keineAhnung", BasicTypeInfo.STRING_TYPE_INFO, ValueTypeInfo.BOOLEAN_VALUE_TYPE_INFO);
           ...
           /*         if (modelBuilder.getHdrf().getCurrentState().getRecord_map().size() > (sampleSize - 500000) && modelBuilder.getHdrf().getCurrentState().getRecord_map().size() < (sampleSize)) {
            ...
            if (this.algorithm.equals("dbh")) {
                if (modelBuilder.getDbh().getCurrentState().checkIfRecordExits(vertex)) {
                    ...
                    modelBuilder.getDbh().getCurrentState().increaseTotalDegree(degree);
                } else {
    ...
    private String removeLowDegreeVertices() {
        ...
        if (this.algorithm.equals("dbh")) {
            double avgDegree = (double) modelBuilder.getDbh().getCurrentState().getAverageDegree();
            int mapSizeBefore = modelBuilder.getDbh().getCurrentState().getRecord_map().size();
            //System.out.println(avgDegree + " avg degree before, size: " + modelBuilder.getDbh().getCurrentState().getRecord_map().size());
            HashSet<Integer> toBeRemoved = new HashSet<>();
            for (int i : this.modelBuilder.getDbh().getCurrentState().getVerticesInStateList()) {
...
/*        if (!timerIsSet) {
       ...
       /* if (modelBuilder.getHdrf().getCurrentState().getRecord_map().size() > 249999) {
        ...
        if (counterEdgesInstance > 0 && counterEdgesInstance % 10_000_000 == 0)
